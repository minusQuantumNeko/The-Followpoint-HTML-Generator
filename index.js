...
for (let i = 0; i < parts.length; i++) {
    if(question_or_answer == 0) {
        ...
        formattedInterviewText += `<a class="avatar news-chat-quote__avatar" href="https://osu.ppy.sh/users/${interviewerID}" style="background-image: url('/wiki/shared/news/${fileName}/avatar-${interviewerName}.jpg')"></a>\n\n<p class="news-chat-quote__username"><a class="news-chat-quote__colour-${interviewerGroup}" href="https://osu.ppy.sh/users/${interviewerID}">${interviewerName}</a></p>\n\n${parts[i].replace(/\*\*\*/mg, "")}\n\n`
        question_or_answer = 1

    } else if(question_or_answer == 1) {

        formattedInterviewText += `<a class="avatar news-chat-quote__avatar" href="https://osu.ppy.sh/users/${intervieweeID}" style="background-image: url('/wiki/shared/news/${fileName}/avatar-${intervieweeName}.jpg')"></a>\n\n<p class="news-chat-quote__username"><a class="news-chat-quote__colour-${intervieweeGroup}" href="https://osu.ppy.sh/users/${intervieweeID}">${intervieweeName}</a></p>${parts[i]}`
